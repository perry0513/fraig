function basic(){
   ...
   for i in ./tests.fraig/*.aag; do
      ...
      # diff output
      # diff (show in terminal)
      diff "myoutput/"$name".out" "myoutput/"$name"-ref.out" |tee "myoutput/"$name".diff"
...
function sweep(){
   ...
   for i in ./tests.fraig/*.aag; do
      ...
      # run program
      ./fraig -f "./mydofile/"$name".do" | grep -v -e "Sweeping" -e "Error" > "myoutput/"$name".out" #2>&1
      ./ref/fraig -f "./mydofile/"$name".do"| grep -v -e "Sweeping" -e "Error" > "myoutput/"$name"-ref.out" #2>&1
      # diff output
      # diff (show in terminal)
      diff "myoutput/"$name".out" "myoutput/"$name"-ref.out" |tee "myoutput/"$name".diff"
      # generate single diff file && append Summary diff file
...
function opt(){
   ...
   for i in ./tests.fraig/*.aag; do
      ...
      # run program
      ./fraig -f "./mydofile/"$name".do" | grep -v -e "Simplifying" > "myoutput/"$name".out" 2>&1
      ./ref/fraig -f "./mydofile/"$name".do" | grep -v -e "Simplifying" > "myoutput/"$name"-ref.out" 2>&1
      # diff output
      # diff (show in terminal)
      diff "myoutput/"$name".out" "myoutput/"$name"-ref.out" |tee "myoutput/"$name".diff"
      # generate single diff file && append Summary diff file
...
function fraig(){
   ...
   for i in ./tests.fraig/*.aag; do
      ...
      # run program
      ./fraig -f "./mydofile/"$name".do" | grep -v -e "Fraig:" -e "SAT:" -e "Total #FEC" -e "Sweeping" -e "Strashing" -e "Simplifying" -e "patterns" > "myoutput/"$name".out"
